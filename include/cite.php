...
$niveaux_autels = 0;
$niveaux_arbres = 0;
$archives = $paquet->get_answer('get_breves')->{1};
...
echo '" class="mtitre">Mythologie</a></div>
	<div class="ligne">';
...
if(sizeof($temples) > 0) {
	...
	if(in_array('demeter', $temples)) {
		...
		' ('.nbf($paquet->get_infoj('furie')).' <img height=\'10px\' src=\'images/attaques/vignette102.gif\' alt="Furies" title="Furies" />)</a><br/>';
	}
	...
	if(in_array('zeus', $temples)) {
		...
		' ('.nbf($paquet->get_infoj('foudre')).' <img height=\'10px\' src=\'images/attaques/zeus.gif\' alt="Foudres" title="Foudres" />)</a><br/>';
	}
	...
	if($points_dispo > 0) {
		if($points_dispo > 1) {
			echo ' <span class="ligne90">('.$points_dispo.' disponibles)</span>';
		}
		else {
			echo ' <span class="ligne90">('.$points_dispo.' disponible)</span>';
		}
	}
...
?><br/>
	...
	if(!empty($bonus->xp) && $bonus->xp > $paquet->get_timestamp()) {
		echo 'Actif jusqu\'au '.display_date($bonus->xp,2);
	}
	else {
		echo ' Inactif';
	}
	...
	if(!empty($bonus->commerce) && $bonus->commerce > $paquet->get_infoj('timestamp')) {
		echo 'Actif jusqu\'au '.display_date($bonus->commerce,2);
	}
	else {
		echo ' Inactif';
	}
	...
	if(!empty($bonus->licence) && $bonus->licence > $paquet->get_timestamp()) {
		echo 'Actif jusqu\'au '.display_date($bonus->licence,2);
	}
	else {
		echo ' Inactif';
	}
