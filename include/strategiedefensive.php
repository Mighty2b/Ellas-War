...
$vague = unserialize($paquet->get_answer('get_vagues')->{1});
$largeur = 4;
...
foreach($liste_unites as $value) {
	if(($value -> defense > 0) && ($value -> nb > 0)) {
		...
		if($value -> nb > 1) {
			$liste .= $unites[$value->nom2]['noms'];
		}
		else {
			$liste .= $unites[$value->nom2]['nom'];
		}
...
if($i%$largeur != 0) {
  ...
  for($j=0;$j<$nb_vague;$j++) {
	  ...
	  <td class="case_suppr_strat droite">&nbsp;<a href="'._(
		'strategiedefensive').'-'.($j+1).'"><img src="images/utils/supprimer_mp.png" 
...
$paquet->error('maj_vagues');

echo '<h1>'._('Stratégie défensive').'</h1><br/>';

if($nb_vague < 50) {
echo '<div style="text-align:right;"> <a href="'._(
		'strategiedefensive').'-0">'._('Ajouter une vague').'</a> </div>';
}
