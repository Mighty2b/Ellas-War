...
<table class="none">
	...
	<tr>
		<td>'.imress('faveur').'</td>
		<td class="droite">'.nbf($paquet->get_infoj('faveur')).'</td>
		<td></td>
	</tr>';
	...
	if($paquet->is_event('stv')) {
		...
		<tr>
		<td>'.imress('stv').'</td>
		<td class="droite">'.nbf($paquet->get_infoj('monnaie')).'</td>
		<td></td>
		</tr>';
	}
...
<div id="inside_co_ress_a">
<a href="'._('details').'" class="details_ress">'._('Détails').'</a>
</div>';
